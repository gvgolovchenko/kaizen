...
Kaizen (改善) — система непрерывного улучшения продуктов v1.22.0. Отслеживает продукты компании, собирает задачи на улучшение (включая асинхронную AI-генерацию через 6 провайдеров с логированием), формирует из них релизы с автоматическим управлением статусами. Поддерживает очередь процессов (QueueManager) с контролем параллелизма по провайдерам, сценарии (ScenarioRunner) для автономных ночных рабочих процессов и планировщик (Scheduler) для автоматического запуска по расписанию.
...
│   ├── scenario-runner.js        # ScenarioRunner — движок выполнения сценариев (batch_develop, auto_release, nightly_audit)
...
│       └── common.js             # Общий скрипт: загрузка и отображение версии в nav-баре
└── docs/
...
| GET | /api/public/roadmap | Публичная дорожная карта (без авторизации): релизы spec/developing/published за N дней (?days=30) |
...
- **Уведомления в Б24**: модуль `notifier.js` отправляет сообщения через бота АФИИНА (ID 1624) методом `im.message.add`. 7 типов событий (release_published, develop_completed/failed, rc_sync_done, gitlab_sync_done, scenario_completed/failed). BB-code форматирование. Интегрирован в process-runner, scheduler, scenario-runner, mcp-server. Per-product настройки через `automation.notifications`. **`postReleaseReport`**: публикует отчёт о релизе в Живую ленту Б24-группы (`log.blogpost.add`, формат `DEST[]=SG{id}`). Задачи группируются по типу с иконками, ссылки на GitLab issues `[url=...]#N[/url]`, приоритеты (🔴🟠🟡), имя AI-модели, время разработки, ссылка на pipeline. Активируется если задано `notifications.b24_group_id`. Триггеры: авто-публикация в process-runner + ручная в api.js.
...
- **Формирование релиза (AI)**: POST /processes с type=form_release → AI группирует открытые задачи в релизы. 4 стратегии (balanced, critical_first, by_topic, single). Авто-утверждение или ручной обзор. UI: rich-модал с summary-баннером (X релизов / Y задач / Z не включены), цветными пипками приоритетов, AI rationale, блоком unassigned. Read-only если релизы уже созданы (`approved_count`, `auto_approved`, или версии совпадают с существующими релизами).
...
- **Моковые данные (seed_data)**: AI-процесс (claude-code). Анализирует `git diff` миграций → находит новые таблицы (INSERT) и новые FK-колонки в существующих таблицах (UPDATE). Получает реальные ID из родительских таблиц. Конфиг: `seed_count` (по умолчанию 20), `tables` (фокус), `use_api` (REST вместо SQL), `branches`. Результат: `records_inserted`, `records_updated`, `tables_seeded`, `tables_updated`.
...
- **Сценарии (ScenarioRunner)**: автономные рабочие процессы = именованная конфигурация pipeline + расписание + история. 4 пресета: `batch_develop` (spec→develop→[run_tests]→[update_docs]→[publish]→[deploy] для списка релизов), `auto_release` (form_release→все предложенные релизы→фильтр auto_approve→spec→develop→[run_tests]→[update_docs]→[publish]→[deploy]), `nightly_audit` (improve→approve для нескольких продуктов), `weekly_digest` (сбор опубликованных релизов за N дней → BB-code пост в Б24-группу `b24_group_id`; без AI-модели, cron `5 9 * * 1` для понедельника 09:05 MSK). Параметры `auto_release`: `min_issues` (мин. задач), `auto_approve` (all/high_and_critical/critical_only), `on_failure` (stop/skip), `develop.*` (enabled/run_tests/update_docs/deploy/auto_publish). **Pre-sync в `auto_release`**: перед поиском открытых задач — GL sync + import + `reopenSyncedIssues` (возврат задач, открытых в GL но закрытых в Kaizen); стадия `gitlab_sync_done {synced, imported, reopened}`. Запуск: сейчас / отложенный (конкретная дата+время MSK) / по cron (регулярно). **Cron хранится и работает в MSK** (локальное время сервера) — никакой конвертации UTC. Cron-сценарии проверяются каждые 60с через scheduler `_runDueScenarios()`. Одноразовые сценарии (с конкретной датой в cron) автоматически отключаются после выполнения. История запусков: `kaizen_scenario_runs` (status, result.stages[], result.summary). Уведомления: `scenario_completed` / `scenario_failed` → Б24. UI: `/scenarios.html` — таблица, фильтры по типу, создание/редактирование с динамической формой, детальная карточка по клику.
...
- **Интеграция с GitLab Issues**: синхронизация issues из GitLab → кэш в kaizen_gitlab_issues → ручной/авто импорт в задачи. Label→type/priority маппинг. MCP: `gitlab_sync`, `gitlab_list_issues`, `gitlab_import_issues`. Автосинхронизация через scheduler (`gitlab_auto_sync` в automation JSONB). Расширенный авто-импорт через `autoImport(productId, config)`: `import_all` (все новые без фильтра), `label_rules` (по меткам), `exclude_labels` (исключить метки), `min_priority` (мин. приоритет через маппинг labels), `ai_enrich.{enabled,model_id}` (improve-процесс после импорта). `close_sync: true` (дефолт) — при sync `state=closed` в GL → Kaizen issue в `done` через `closeSyncedIssues(productId)`. **`reopenSyncedIssues(productId)`** — обратная операция: если GL issue `state=opened`, а Kaizen issue `status=done` → возвращает в `open` (используется в `auto_release` pre-sync).
- **Обратная синхронизация GitLab-лейблов (issue → done)**: при переходе kaizen-задачи в `done` c заполненным `gitlab_issue_id` автоматически обновляются метки связанного GitLab issue — снимаются `В работе` и `Требуется доработка`, ставится `Разработка завершена` (`DEVELOPED_LABELS` в `gitlab-client.js`), добавляется комментарий «🧪 Разработка завершена в Kaizen — готово к тестированию». **GL issue НЕ закрывается** — остаётся open, чтобы тестировщик увидел задачу утром. Точки интеграции: `PUT /api/issues/:id` (ручной апдейт) и `releases.publish()` (массовое закрытие при публикации релиза — покрывает auto-publish в develop_release, сценарии `batch_develop` / `auto_release`). Реализация: `markIssueDeveloped(deploy, issueIid)` + батч-хелпер `syncIssuesDeveloped(deploy, issues)` (fire-and-forget, Promise.allSettled).
...
- **48 инструментов** с префиксом `kaizen_`:
  ...
  - Моковые данные: `seed_data` (через `POST /processes` с `type: 'seed_data'`)
